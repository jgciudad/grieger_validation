...
class Model(nn.Module):
    ...
    feature extractor with 8 conv layers, BatchNorm, Leaky ReLU activations and dropout layers; the dropout
    probabilities can be set in the config file of the experiment (FEATURE_EXTR_DROPOUT)

    classifier consists of 2 dropout layers followed by linear layers with a Leaky ReLU activation after the first
    layer; the dropout probabilities can be set in the config file of the experiment as well (CLASSIFIER_DROPOUT)
    """
